...
int main(int argc, char *argv[])
{
  ...
  // double *temp_u = (double *)calloc(n, sizeof(double));
  // if (temp_u == NULL) {
...
for (i = 0; i < n;i++) {
  ...
  #pragma omp parallel
  {
    ...
    for (it = 0; it < itime; it++) {
      #pragma omp task private(i,j) depend(out:sum)
      {
        // printf("Task1: Thread : %d it: %d i: %d\n", omp_get_thread_num(), it, i);
      for (i = 0; i < n; i++) {
  ...
  // #pragma omp taskwait
    ...
    if (it % ntstep == 0) {
      ...
      #pragma omp task private(i) depend(in:sum)
      {
        printf("Time is %ld\n", it);
      ...
      #pragma omp task private(j) depend(in:sum)
      {
