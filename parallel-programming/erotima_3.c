...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  int end ;
   int index = 0;
   int time_index = 0;
   double  *spacetime_buf;//buffer gia to spacetime
   spacetime_buf = (double *)calloc(n * itime, sizeof(double));
   double  *omega_buf;//buffer gia to omega
   omega_buf = (double *)calloc(n * itime, sizeof(double));
   long int  *time_buf;
   time_buf = (long int *)calloc(itime, sizeof(long int));
   ...
   #pragma omp parallel private(it,i,j,step) firstprivate(n,sigma,dt,mu,semi_sum)
   {
     ...
     for (it = 0; it < itime; it++) {
       ...
       if (it % ntstep == 0) {
         ...
         #pragma omp master
         {
           ...
           time_buf[time_index] = it;
           time_index++;
           // printf("time buf: %ld index: %d\n",time_buf[index],index);
           time = (double)it * dt;
           ...
           for (i = 0; i < n; i++) {
             ...
             spacetime_buf[index] = u[i];
             omega_buf[index] = omega[i];
             index++;
           }
     ...
     #pragma omp single
     {
       ...
       fprintf(output1, "%ld\t",   time_buf[0]);
       fprintf(output2, "%ld\t",   time_buf[0]);

       #pragma omp task firstprivate(index)
       {
         int time_index1 = 1;
         // printf("thread to space: %d\n", omp_get_thread_num());
         for (int i = 0; i < index; i++) {
           ...
           fprintf(output1, "%19.15f",   spacetime_buf[i]);
         }
       }
       ...
       #pragma omp task firstprivate(index)
       {
         ...
         for (int i = 0; i < index; i++) {
           ...
           fprintf(output2, "%19.15f",   omega_buf[i]);
